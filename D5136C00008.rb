...
class D5136C00008_Site
  ...
  def reader(inbound_file)
    @logger.debug "File name is ->#{inbound_file}<-"
    @logger.info "#{self.class.name} reader start"
    @inbound_lines = CSV.read(inbound_file, field_size_limit: 1000, headers: true, skip_blanks: true, skip_lines: '\r', encoding:'windows-1256:utf-8')
    @logger.info "#{self.class.name} reader end"
    @inbound_lines.length
  end
...
class D5136C00008_Inventory
  VISIT_MAP = {
      :Visit1               => 'Visit 1',
      :Visit2               => 'Visit 2',
      :Visit3               => 'Visit 3',
      :Visit4               => 'Visit 4',
      :Visit5EndOfTreatment => 'Visit 5',
      :Retest               => 'Visit 6',
  }.freeze
  ...
  def writer(vendor)
    ...
    @processing_lines.each do |outline|
      ...
      visit_name         = (outline[25].nil?)  ? ' NULL,' : VISIT_MAP[outline[25].gsub(/\s+/, '').to_sym].insert_value
      ...
      specimen_condition = (outline[22].nil?)  ? ' NULL,' : SPECIMEN_CONDITION[outline[22].gsub(/\s+/, '').to_sym].insert_value
      specimen_status    = (outline[18].nil?)  ? " 'Exhausted'," : SPECIMEN_STATUS[outline[18].gsub(/\s+/, '').to_sym].insert_value

      if outline[15].nil?
        designation = 'NULL,'
      elsif outline[15].start_with? 'SM'
        designation = "'PK',"
      elsif outline[15].start_with? 'RM'
        designation = "'Biomarker',"
      else
        designation = 'NULL,'
      end
...
class D5136C00008_Assay
  ...
  def processor(this_connection)
    ...
    @inbound_lines.each do |specline|
      ...
      if (specline[11].nil? || specline[11] == '') &&
         (specline[12].nil? || specline[12] == '')
        @logger.info '================= QUERY NEED ===================='
