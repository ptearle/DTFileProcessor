...
#     subject_treatment   = (outline[9].nil?)  ? ' NULL,' : "#{TREATMENT[('T' + outline[9][2..3]).to_sym]}".insert_value
      subject_treatment   = 'Part A'.insert_value
      subject_arm         = (outline[9].nil?)  ? ' NULL,' : "#{ARM[outline[9].to_sym]}".insert_value
...
class R1193_DM_1402_RGRNInv
  ...
  def processor(this_connection)
    ...
    my_select = "SELECT   MAX(dsv.id),
                 ...
                 WHERE    dsv.study_protocol_id = 'R1193-DM-1402'
